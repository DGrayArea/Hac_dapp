...
const UnstakedPagination = ({ contract, cardName, vaultId, tokenC }) => {
  ...
  const { contract: stakeContract } = useContract(
    "0x60a00a6d716e2c2F896698B28dC1a7a508f0b87A"
  );
  ...
  const callStake = async () => {
    ...
    try {
      const data = await stake([vaultId, selected, address]);
  ...
  const callStakeAll = async () => {
    ...
    try {
      const data = await stake([vaultId, ownedNFTs, address]);
      console.info("contract call successs", data);
  ...
  return (
    <div className="mx-auto">
      ...
      <div className="grid grid-cols-2  md:hidden space-y-3 space-x-2 justify-center text-center mr-2 items-center">
