...
const UnstakedPagination = ({ contract, cardName, vaultId }) => {
    ...
    const { contract: stakeContract } = useContract('0x4d2Bfa7d1D13e6C13A08a92B32a238b2B6DFFE0E')
  
  
    const { data: isApproved, isLoading: isLoadingApproved } = useContractRead(contract, "isApprovedForAll", address, '0x4d2Bfa7d1D13e6C13A08a92B32a238b2B6DFFE0E')
    ...
    const callApprove = async () => {
      ...
      try {
        const data = await setApprovalForAll([ '0x4d2Bfa7d1D13e6C13A08a92B32a238b2B6DFFE0E', true ]);
        console.info("contract call successs", data);
...
getUri(parseInt(post))
       ...
       <Grid xs={3} key={parseInt(post)}>
        <Container css={{mr:'$36'}}>
          <Card style={{border:'1px solid gray'}} onClick={(e) => check(parseInt(post), e)} variant='bordered' css={{width: '150px', height:"185px"}} isPressable>
            <Card.Body css={{ p: 0 }}>
