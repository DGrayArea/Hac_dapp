...
const UnstakedPagination = ({ contract, cardName, vaultId }) => {
   ...
   <div className='hidden mx-auto md:items-center md:text-center md:justify-center md:space-x-3 lg:items-center lg:text-center md:ml-16 lg:ml-40 lg:justify-center md:block lg:block'>
   <Grid.Container className='mx-auto flex justify-center items-center text-center' gap={2} justify='center'>
      {currentPosts?.map(post => (
       <Grid xs={4} key={post.metadata.id}>
        <div onMouseOver={(e) => {console.log(e)}} className='highlight' style={{margin:"auto",marginRight:'80px', borderRadius:'16px'}}>
          <Card onClick={(e) => check(post.metadata.id, e)} variant='bordered' css={{width: '150px', height:"185px"}} isPressable>
            <Card.Body css={{ p: 0 }}>
