...
const UnstakedPagination = ({ contract, vaultId }) => {
  ...
  useEffect(() => {
    ...
    const callNtt = async () => {
      if (vaultId == 0) {
        const data = await fetch(`${API_URL}/details/333/${address}`, {
          signal,
        });
        ...
        const data = await fetch(`${API_URL}/details/3333/${address}`);
        const res = await data.json();
        setOwnedNFTs(res);
      } else if (vaultId == 2) {
        const data = await fetch(`${API_URL}/details/aliens/${address}`);
        const res = await data.json();
        setOwnedNFTs(res);
      }
      const isApp = await fetch(`${API_URL}/approved/${address}`).then((res) =>
        res.json()
      );
  ...
  const { write: stake, isSuccess: loadingStake } =
    useContractWrite(stakeSelected);
  ...
  const {
    data,
    write: stakeAll,
    isSuccess: loadingStakeAll,
  } = useContractWrite(stakeall);
      ...
      <div className="md:hidden justify-center text-center mr-2 items-center">
        <Grid.Container className="grid grid-cols-2  md:hidden space-y-3 space-x-2 justify-center text-center mr-2 items-center">
          {currentMobilePosts?.map((post) => {
            ...
            const imge = image(parseInt(post) - 1).then((res) => {
              let x = res.replace("ipfs://", "https://ipfs.io/ipfs/");
              document.getElementById("token" + parseInt(post).toString()).src =
                x;
            });
            return (
