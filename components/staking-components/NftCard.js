...
const NftCard = ({ nftData, loading, mobileData, contract, cardName, vaultId }) => {
  ...
  const { contract: stakeContract } = useContract('0xef5F57d5b7635DD7a825e6089ace9231f84C855e')


  const { data: isApproved, isLoading: isLoadingApproved } = useContractRead(contract, "isApprovedForAll", address, '0xef5F57d5b7635DD7a825e6089ace9231f84C855e')
  ...
  const callApprove = async () => {
    try {
      const data = await setApprovalForAll([ '0xef5F57d5b7635DD7a825e6089ace9231f84C855e', true ]);
      console.info("contract call successs", data);
    } catch (err) {
      console.error("contract call failure", err);
    }
