...
const NftCard = ({ nftData, loading, mobileData, contract, cardName, vaultId }) => {
  ...
  const { contract: stakeContract } = useContract('0x76F69BE8739b4B0D8A23c498b880614aA01bA91B')


  const { data: isApproved, isLoading: isLoadingApproved } = useContractRead(contract, "isApprovedForAll", address, '0x76F69BE8739b4B0D8A23c498b880614aA01bA91B')
  ...
  const callApprove = async () => {
    ...
    try {
      const data = await setApprovalForAll([ '0x76F69BE8739b4B0D8A23c498b880614aA01bA91B', true ]);
      console.info("contract call successs", data);
