...
const NFTs = () => {
  ...
  useEffect(() => {
    ...
    const bal = async() => {
        const data = await fetch(`${API_URL}/balances/userbal/${address}`, {signal})
      ...
      const data333 = await fetch(`${API_URL}/staking/333/${address}`);
      const res333 = await data333.json();
      set333Staked(res333);
      
      const data3333 = await fetch(`${API_URL}/staking/3333/${address}`);
      const res3333 = await data3333.json();
      set3333Staked(res3333);

      const dataAlien = await fetch(`${API_URL}/staking/aliens/${address}`);
