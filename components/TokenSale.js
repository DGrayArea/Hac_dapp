...
import Nav from './Nav'
import { useContractRead, useAddress, useContractWrite, useContract } from "@thirdweb-dev/react";
...
const TokenSale = () => {
  ...
  const { contract } = useContract("0x56628775730cA381557b6B73Cf7CC54f981bB722");
  const { data: prices } = useContractRead(contract, "getLatestPrice")
  ...
  const callBuy = async () => {
    ...
    try {
      const data = await buyTickets([ quantity, x, {value: ticketprice} ]);
  ...
  async function fetchData() {
   ...
   const data = await fetch(`/api/tokensale`).then(res => res.text()).then((d) => {
    setPrice((parseInt(d)/1E18).toFixed(4))
    setFuncPrice(d)
  })
  }
  ...
  const TooltipContent = () => {
    return (
        ...
        <i>
        Please Note: Each 'BNB' balance on a staking page is separate to each collection, and that collections stake-to-win lottery. Alien Horror Apes BNB cannot be used for Project 333 Lotteries, and vice versa
        </i>
      </p>
    )
  }
        ...
        <div className='stats-container mb-6 space-y-2'>
          <div className="stats-container">
            <div className='flex extra justify-between items-center text-white pb-2'>
              <h4 className='extra'>Price Per <span id='ticket-name'>333</span> Ticket</h4>
